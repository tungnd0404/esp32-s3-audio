...
static uint16_t decodeNow  = 0;              // giá trị decode_time hiện tại
...
void initSync() {
    lastSync = millis();
    decodePrev = VS1053_ReadRegister(SCI_DECODE_TIME);
...
static int get_virtualtime() {
    ...
    while (now - lastSync >= 100) {
        ...
        uint16_t t_raw = VS1053_ReadRegister(SCI_DECODE_TIME);
